...
class Result:
    def __init__(self, image, detect_label, detections):
        self.image = image
...
def image_detection(image_or_path, network, class_names, class_colors, thresh):
    ...
    return image, detections, image_shape, image_rgb
...
def main_detect(input, network, class_names, class_colors):
    ...
    while True:
        ...
        image, detections, image_shape, image_rgb = image_detection(
        ...
        for label, confidence, bbox in sorted_detections:
            if label == '1':
                ...
                #cv2.imshow("test_image", resized_image)
                test_images = resized_image / 255.0
                test_images[test_images > 0.1] = 1.0
        ...
        res.append(Result(image, l, sorted_detections))
